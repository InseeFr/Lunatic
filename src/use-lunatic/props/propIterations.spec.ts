...
describe('propIterations', () => {
	...
	it('compute the max iterations from the child components', () => {
		const components: LunaticComponentDefinition = {
			id: 'm1azzo4j',
			page: '3',
			lines: {
		...
		const variables = LunaticVariablesStore.makeFromObject({
			NAME: ['Nergigante', 'Rathalos', null],
		});
		...
		expect(getIterationsProp(components, mockState)).toBe(2);
	});
});
